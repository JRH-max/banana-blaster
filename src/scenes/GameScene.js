  ...
  _updateCar(dt) {
    ...
    if (this.inCar) {
      ...
      if (this.joystickActive) {
        // Joystick: drive in whatever direction it points
        const inputX = this.joystickDir.x;
        const inputY = this.joystickDir.y;
        const inputLen = Math.hypot(inputX, inputY);
        if (inputLen > 0.05) {
          const nx2 = inputX / inputLen, ny2 = inputY / inputLen;
        ...
        // Keyboard: classic throttle + steer
        const throttle = kbUp ? 1 : kbDown ? -0.5 : 0;
        const steer    = kbLeft ? -1 : kbRight ? 1 : 0;
        this.carSpeed += throttle * ACCEL * dt;
        this.carSpeed *= (1 - dt * (hasKbInput ? st.friction : st.friction * 2.2));
        this.carSpeed = Phaser.Math.Clamp(this.carSpeed, -MAX_SPEED * 0.45, MAX_SPEED);
        if (Math.abs(this.carSpeed) > 0.3) car.angle += steer * 2.0 * dt * Math.sign(this.carSpeed);
